...
(function () {
  ...
  var zoneByTitle = {};      // title -> {id, path, sessionIds}
  ...
  var CSS = ''
    ...
    + '.hub-zone-del{flex:none;border:0;background:transparent;cursor:pointer;font-size:12px;'
  ...
  function injectDeleteButtons() {
    for (var i = 0; i < zoneTitles.length; i++) {
      ...
      var prev = opBtn.previousElementSibling;
      if (prev && prev.classList && prev.classList.contains('hub-zone-del')) { continue; }
      if (opBtn.parentElement.querySelector(':scope > .hub-zone-del[data-zone="' + title + '"]')) { continue; }
      ...
      opBtn.parentElement.insertBefore(del, opBtn);
    }
  }
